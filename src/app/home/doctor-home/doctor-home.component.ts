...
/**
 * Doctor's Home page ("Today's appointments," feature 010): every one of the Doctor's own
 * appointments dated today and still SCHEDULED — nothing from other days, and no
 * past/cancelled/completed section at all (that's what the Appointments tab is for).
 *
 * Feature 012: also surfaces a separate "Overdue appointments" section, above "Today's
 * appointments," for any of the Doctor's own SCHEDULED appointments whose date has already
 * passed — flagged with an alert icon/tooltip and resolvable in place via the same
 * Complete/Cancel actions `AppointmentsListComponent` already offers (research.md #4/#6).
 *
 * Feature 013: requests only SCHEDULED appointments dated on or before today directly from the
 * server (covering both sections above from one small response) instead of fetching the doctor's
 * whole appointment history and filtering it here (research.md #7 of feature 013).
 */
@Component({
