...
describe('ClinicAdminHomeComponent', () => {
  ...
  function setup(appointments: AppointmentResponse[]) {
    const searchClinicAppointmentsSpy = vi.fn().mockReturnValue(
    ...
    return { fixture, searchClinicAppointmentsSpy };
  }

  it("requests only SCHEDULED appointments on or before today, not the clinic's entire history (feature 013)", () => {
    const { fixture, searchClinicAppointmentsSpy } = setup([appointment({ id: '1' })]);
  ...
  it('renders appointments with patient and doctor names (feature 008)', () => {
